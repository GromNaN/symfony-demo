...
Ce support `autoEmbed` dans Doctrine MongoDB ODM n'existe pas encore officiellement : il vient d'une [pull request non mergée](https://github.com/doctrine/mongodb-odm/pull/3024), installée via un repository VCS Composer pointant sur la branche de la PR. Ce n'est pas une contrainte gênante en soi — c'est même l'occasion de tester une fonctionnalité avant sa sortie — mais ça vaut le coup de le savoir avant de partir sur ce projet en production.
