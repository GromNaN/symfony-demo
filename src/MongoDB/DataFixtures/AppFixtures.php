...
final class AppFixtures implements FixtureInterface
{
    ...
    public function load(Repository $repository): void
    {
        $this->loadUsers($repository);
        $this->loadTags($repository);
        $this->loadPosts($repository);
    }

    private function loadUsers(Repository $repository): void
    {
    ...
    private function loadTags(Repository $repository): void
    {
    ...
    private function loadPosts(Repository $repository): void
    {
        foreach ($this->getPostData() as [$title, $slug, $summary, $content, $publishedAt, $author, $tags]) {
            ...
            foreach (range(1, 5) as $i) {
                ...
                $comment = new Comment(
                    ...
                    post: $post,
                    publishedAt: new \DateTimeImmutable('now + '.$i.'seconds'),
                );
    ...
    final protected function addReference(string $key, object $object)
    {
        if (isset($this->references[$key])) {
            throw new \LogicException(sprintf('Reference "%s" already exists.', $key));
        }

        $this->references[$key] = $object;
    }

    final protected function getReference(string $key): object
    {
        if (!isset($this->references[$key])) {
            throw new \LogicException(sprintf('Reference "%s" does not exist.', $key));
        }
